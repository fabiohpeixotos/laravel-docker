...
pre-build:
	$(info ${Y} Pre-building constants ${R})
	$(info ${Y} clone .env file ${R})
	cp .env.example .env	
	$(info ${Y} Create log folder in system ${R})
...
composer-install: 
	...
	U_ID=${UID} docker-compose run --rm --user ${UID} php composer install --no-scripts --no-interaction --optimize-autoloader
